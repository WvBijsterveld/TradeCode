...
def main():
    ...
    if args.mode == "rl":
        ...
        if model_in_dim is not None and int(model_in_dim) != int(expected_in_dim):
            raise SystemExit(
                "Supervised model input dim mismatch: "
                f"model expects {int(model_in_dim)} but current config produces {int(expected_in_dim)} "
                f"(n_features={int(n_features)}, window={int(WINDOW_SIZE)}). "
                "This usually means a missing feature flag (e.g., add --ifvg-zone-features if your .pt was trained with it)."
            )
    ...
    while True:
        ...
        try:
            ...
            if obs is None:
                # Still log the bar so users can verify the bridge is receiving data.
                ...
                log_sink.write_row(
                    {
                        ...
                        "bias": int(getattr(builder, "day_bias", 0)),
                        "atr_pips": ("" if not np.isfinite(getattr(builder, "last_atr_pips", float("nan"))) else f"{float(builder.last_atr_pips):.4f}"),
                        "trend_slope_atr": (
                            ""
                            if not np.isfinite(getattr(builder, "last_trend_slope_atr", float("nan")))
                            else f"{float(builder.last_trend_slope_atr):.4f}"
                        ),
                        ...
                        **(
                            {
                                ...
                                "sup_thresh_long": f"{_resolve_sup_thresholds(args=args, sup_meta=sup_meta)[0]:.4f}",
                                "sup_thresh_short": f"{_resolve_sup_thresholds(args=args, sup_meta=sup_meta)[1]:.4f}",
                                "choose_long": False,
                                "choose_short": False,
                            }
                            if args.mode == "supervised"
            ...
            # Hot-reload supervised weights if enabled.
            if args.mode == "supervised" and bool(args.sup_hot_reload):
            ...
            if args.mode == "rl":
                ...
                assert sup_long is not None and sup_short is not None
                ...
                # Adaptive quality gate: if we're in a losing streak / cooldown, demand higher confidence.
                ...
                # Enforce a minimum confidence filter if requested.
                try:
                ...
                # Optional: only trade when the model clearly prefers one side.
                # Supports per-side overrides (useful when one direction is structurally weaker).
                try:
                ...
                # During cooldown, always hold (trade less; higher quality only).
